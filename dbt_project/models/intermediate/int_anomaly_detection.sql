...
--   3) duplicate_mmsi_timestamp: multiple rows share (mmsi, basedatetime)
-- Output columns: mmsi, event_time, anomaly_type, details (jsonb), created_at
-- Notes:
--   - event_time uses basedatetime exactly as produced by stg_ais_cleaned
--   - COG math uses numeric to match stg_ais_cleaned.cog (decimal(5,2))
--   - Index helps typical lookups by (mmsi, event_time)
{{ config(
    materialized='table',
    indexes=[
      { 'columns': ['mmsi', 'event_time'] }
    ]
) }}
...
ordered as (
  select
    ...
    basedatetime - lag(basedatetime) over (partition by mmsi order by basedatetime) as dt
  from base
),
...
-- Find (mmsi, basedatetime) groups with more than one record
dups as (
  select mmsi, basedatetime as event_time, count(*) as cnt
  from base
  group by 1,2
  having count(*) > 1
),
